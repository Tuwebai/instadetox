...
/** Fallback tipo Instagram cuando el usuario no tiene avatar. */
...
const MessagesNewChatModal = ({
  ...
  const selectedUsers = useMemo(
    () =>
      suggestions.filter((row) => selectedIds.includes(row.id)),
  ...
  return (
    <div
      className="ig-dm-new-chat-backdrop"
      role="presentation"
      onClick={onClose}
    >
      <section
      ...
      >
        <header className="ig-dm-new-chat-head">
          ...
          <button
            type="button"
            className="ig-dm-new-chat-close"
            onClick={onClose}
            aria-label="Cerrar"
          >
            <svg
        ...
        <div className="ig-dm-new-chat-to">
          <label
            htmlFor="ig-dm-new-chat-query"
            className="ig-dm-new-chat-to-label"
          >
            Para:
          </label>
          <div className="ig-dm-new-chat-query-wrap">
            {selectedUsers.map((user) => (
              <div key={user.id} className="ig-dm-new-chat-tag">
                <span className="ig-dm-new-chat-tag-text">
                  {user.fullName || user.username}
                </span>
                <button
                  type="button"
                  className="ig-dm-new-chat-tag-close"
                  aria-label={`Eliminar a ${user.fullName || user.username}`}
                  onClick={() => {
                    setSelectedIds((prev) =>
                ...
                >
                  <svg
                    ...
                    width="12"
                  >
                    <title>{`Eliminar a ${user.fullName || user.username}`}</title>
                    <polyline
                      fill="none"
                      points="20.643 3.357 12 12 3.353 20.647"
                      stroke="currentColor"
        ...
        <hr className="ig-dm-new-chat-sep" aria-hidden="true" />

        <div className="ig-dm-new-chat-list-wrap">
          <h3 className="ig-dm-new-chat-suggestions-title">Sugerencias</h3>
          <div className="ig-dm-new-chat-list">
            {loading ? <p className="ig-dm-empty-list">Buscando...</p> : null}
            {!loading && suggestions.length === 0 ? (
              <p className="ig-dm-empty-list">Sin resultados.</p>
            ) : null}
            {!loading
              ? suggestions.map((user) => {
                  ...
                  return (
                    ...
                    >
                      <img
                        src={user.avatarUrl ?? AVATAR_FALLBACK_URL}
                        alt=""
                        width={44}
                        height={44}
                      />
                      ...
                      <span
                        className={`ig-dm-new-chat-check${selected ? " is-on" : ""}`}
                        aria-hidden="true"
                      >
                        {selected ? (
                          <span className="ig-dm-new-chat-check-dot" />
                        ) : null}
                      </span>
                    </button>
                  );
